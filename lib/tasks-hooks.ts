...
export function useTasks(userId: string | null, status?: string) {
  ...
  const { data, error, isLoading, mutate } = useSWR(userId ? `/api/tasks?${query.toString()}` : null, fetcher, {
...
export async function createTaskAction(userId: string, title: string, description: string, dueDate: string) {
